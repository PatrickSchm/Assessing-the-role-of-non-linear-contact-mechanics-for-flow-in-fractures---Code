                         ...
                         const LFSV &lfsv, MA &mat, int iMat) const
    {
             ...
             ip != rule.end(); ++ip)
        {
            ...
            for (size_type i = 0; i < usize; i++)
            {
                ...
                // {
                //     mat.accumulate(lfsu_ur, i, plfsu, j,
                //                    rad * (-alpha * gradphi[i][0] * psi[j]
                //                          )* factor);
                //     mat.accumulate(lfsu_uz, i, plfsu, j,
                //                    rad * (-alpha * gradphi[i][1] * psi[j]
                //                          )* factor);
                // }
            }
            for (size_type i = 0; i < psize; i++)
                              ...
                              const LFSV &lfsv, R &r, std::vector<int> indices) const
    {
             ...
             ip != rule.end(); ++ip)
        {
            ...
            if (zt_element.topIndx == zt_element.active_inter)
            {
                ...
                //     int iT = zt_element.matchTopMap[1];
                //     int iB = zt_element.matchButtomMap[1];
                ...
                //     double delta_boundary = xdelta[iT];
                //     double Q_inp = Kf * (Q_bd / (2.0 * 3.142 * 1000.0 * 60.0 * 0.1));
                //     // double Q_inp = Kf  * ;
                //     // double Q_inp = Kf * 0.1;
                //     // 1.4 instead of 1./2 because it ierates over integration points of element - here hard coded neumann for nodes from surface
                //     r.accumulate(plfsu,   iT,  -1.0 / 4.0 * 0.1 * Q_inp );
                //     r.accumulate(plfsu,   iB,  -1.0 / 4.0 * 0.1 * Q_inp );
                // }
             ...
             ip != rule.end(); ++ip)
        {
            ...
            for (size_type i = 0; i < 2; ++i)
            {
                ...
                for (size_type j = 0; j < 2; ++j)
                {
                    ...
                    // double E_spring_dz_top = 1.0 / (delta_new * delta_new) * EFrac * phi[jT] * zt_element.topNormal[1];
                    // double E_spring_dz_buttom = 1.0 / (delta_new * delta_new) * EFrac * phi[jT] * zt_element.buttomNormal[1];
