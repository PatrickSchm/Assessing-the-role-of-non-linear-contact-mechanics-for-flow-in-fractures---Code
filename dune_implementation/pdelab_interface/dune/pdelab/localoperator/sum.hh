...
private:
  ...
  struct AlphaZeroThicknessOperation {
    template<typename EG, typename IGS, typename LFSU, typename X, typename LFSV,
             typename R>
    static void apply(const ArgPtrs& lops, const EG& eg, const IGS& igs,
                      const LFSU& lfsu, const X& x, const LFSV& lfsv,
                      R& r, int iMat)
    {
      LocalAssemblerCallSwitch<typename tuple_element<i, Args>::type,
                               tuple_element<i, Args>::type::doZeroThickness>::
                               alpha_zero_thickness(*get<i>(lops), eg, igs,  lfsu, x, lfsv, r, iMat);
    }
  };
...
public:
  ...
  ( const EG& eg, const IGS& igs,
    ...
    R& r, int iMat) const
  {
    ForLoop < AlphaZeroThicknessOperation, 0, size - 1 >::
    apply(lops, eg, igs, lfsu, x, lfsv, r);
  }
...
private:
  ...
  struct JacobianZeroThicknessOperation {
                      ...
                      LocalMatrix& mat)
    {
      ...
      LocalAssemblerCallSwitch<typename tuple_element<i, Args>::type,
                               tuple_element<i, Args>::type::doZeroThickness>::
                               jacobian_zero_thickness(*get<i>(lops), eg, igs, lfsu, x, lfsv, mat);
    }
  };
...
public:
  ...
  ( const EG& eg, const IGS& igs,
    ...
    LocalMatrix& mat) const
  {
    ForLoop < JacobianZeroThicknessOperation, 0, size - 1 >::
    apply(lops, eg, igs, lfsu, x, lfsv, mat);
  }
