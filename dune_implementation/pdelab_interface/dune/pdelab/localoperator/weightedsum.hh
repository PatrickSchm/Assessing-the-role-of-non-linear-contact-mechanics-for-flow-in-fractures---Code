...
private:
  ...
  template<int i>
  struct AlphaZeroThicknessValue : public std::integral_constant
    < bool, tuple_element<i, Args>::type::doZeroThickness>
  { };
  template<int i>
...
public:
  ...
  //! \brief Whether to call the local operator's alpha_volume(),
  //!        jacobian_apply_volume() and jacobian_volume().
  enum { doZeroThickness              =
           AccFlag<AlphaZeroThicknessValue>::value
       };
...
private:
  ...
  template<int i>
  struct PatternZeroThicknessOperation {
    template<typename LFSU, typename LFSV, typename LocalPattern>
    static void apply(const ArgPtrs& lops, const Weights& weights,
                      const LFSU& lfsu, const LFSV& lfsv,
                      LocalPattern& pattern)
...
private:
  ...
  template<int i>
  struct AlphaZeroThicknessOperation {
    typedef typename tuple_element<i, Args>::type Arg;
    ...
    static void apply(const ArgPtrs& lops, const Weights& weights,
                      ...
                      WeightedVectorAccumulationView<C>& r)
    {
      apply(lops, weights[i]*r.weight(), eg, igs, lfsu, x, lfsv, r.container());
    }
    template<typename EG, typename IGS, typename LFSU, typename X, typename LFSV,
             typename C>
    static void apply(const ArgPtrs& lops, typename C::weight_type weight,
                      ...
                      C& r)
    {
      if (weight != K(0)) {
        WeightedVectorAccumulationView<C> view(r, weight);
        LocalAssemblerCallSwitch<Arg, Arg::doAlphaVolume>::
        alpha_volume(*get<i>(lops), eg, igs,  lfsu, x, lfsv, view);
      }
    }
  };
...
public:
  ...
  template<typename EG, typename IGS, typename LFSU, typename X, typename LFSV,
           typename C>
  void alpha_zero_thickness
  ( const EG& eg, const IGS& igs,
    const LFSU& lfsu, const X& x, const LFSV& lfsv,
    C& r) const
  {
    ForLoop < AlphaVolumeOperation, 0, size - 1 >::
    apply(lops, weights, eg, igs, lfsu, x, lfsv, r);
  }
...
private:
  ...
  template<int i>
  struct JacobianApplyZeroThicknessOperation {
    typedef typename tuple_element<i, Args>::type Arg;
    template<typename EG, typename IGS, typename LFSU, typename X, typename LFSV,
             typename C>
    static void apply(const ArgPtrs& lops, const Weights& weights,
                      ...
                      C& r)
    {
      ...
      if (weight != K(0)) {
        WeightedVectorAccumulationView<C> view(r, weight);
        LocalAssemblerCallSwitch<Arg, Arg::doAlphaVolume>::
        jacobian_apply_zero_thickness(*get<i>(lops), eg, igs, lfsu, x, lfsv, view);
      }
    }
  };
...
private:
  ...
  template<int i>
  struct JacobianZeroThicknessOperation {
    typedef typename tuple_element<i, Args>::type Arg;
    template<typename EG, typename IGS, typename LFSU, typename X, typename LFSV,
             typename C>
    static void apply(const ArgPtrs& lops, const Weights& weights,
                      ...
                      WeightedMatrixAccumulationView<C>& m)
    {
      apply(lops, weights[i]*m.weight(), eg, igs, lfsu, x, lfsv, m.container());
    }
    template<typename EG, typename IGS, typename LFSU, typename X, typename LFSV,
             typename C>
    static void apply(const ArgPtrs& lops, typename C::weight_type weight,
                      ...
                      C& m)
    {
      ...
      if (weight != K(0)) {
        WeightedMatrixAccumulationView<C> view(m, weight);
        LocalAssemblerCallSwitch<Arg, Arg::doZeroThickness>::
        jacobian_zero_thickness(*get<i>(lops), eg, igs, lfsu, x, lfsv, view);
      }
    }
  };
...
public:
  ...
  {
    ForLoop < JacobianVolumeOperation, 0, size - 1 >::
    apply(lops, weights, eg, igs, lfsu, x, lfsv, m);
  }
