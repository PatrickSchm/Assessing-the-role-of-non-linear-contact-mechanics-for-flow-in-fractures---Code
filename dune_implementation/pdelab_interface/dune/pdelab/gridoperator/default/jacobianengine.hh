#ifndef DUNE_PDELAB_GRIDOPERATOR_DEFAULT_JACOBIANENGINE_HH
#define DUNE_PDELAB_GRIDOPERATOR_DEFAULT_JACOBIANENGINE_HH
...
public:
  ...
  */
  ...
  { return local_assembler.doZeroThickness(); }
  ...
  template<typename EG, typename LFSUC, typename LFSVC>
  void assembleUVVolume(const EG & eg, const LFSUC & lfsu_cache, const LFSVC & lfsv_cache, int iMat)
  {
    al_view.setWeight(local_assembler.weight());
    Dune::PDELab::LocalAssemblerCallSwitch<LOP, LOP::doAlphaVolume>::
    jacobian_volume(lop, eg, lfsu_cache.localFunctionSpace(), xl, lfsv_cache.localFunctionSpace(), al_view, iMat);
  }

  template<typename EG, typename ITS, typename LFSUC, typename LFSVC>
  void assembleZeroThickness(const EG & eg, const ITS & igs, const LFSUC & lfsu_cache, const LFSVC & lfsv_cache, std::vector<int> indices)
  {
    al_view.setWeight(local_assembler.weight());
    Dune::PDELab::LocalAssemblerCallSwitch<LOP, LOP::doZeroThickness>::
    jacobian_zero_thickness(lop, eg, igs, lfsu_cache.localFunctionSpace(), xl, lfsv_cache.localFunctionSpace(), al_view, indices);
  }
...
#endif // DUNE_PDELAB_GRIDOPERATOR_DEFAULT_JACOBIANENGINE_HH
