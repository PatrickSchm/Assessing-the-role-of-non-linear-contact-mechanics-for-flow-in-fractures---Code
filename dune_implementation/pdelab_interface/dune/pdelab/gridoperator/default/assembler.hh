...
std::map<int, ZeroThickness> zTContainer;
std::vector<bool> zTBool(1e6, false);
  ...
  void assemble(LocalAssemblerEngine & assembler_engine) const
  {
    ...
    for (const auto& element : elements(entity_set))
    {
      ...
      } else
      {
        ...
        // if (zTBool[idsindex] == false)
        //   setInterface(idsindex, entity_set, element);

        // if (zTBool[idsindex] == false)
        auto zt_element =  setInterface(idsindex, entity_set, element);
        ...
        {
          // ZeroThickness zt_element = zTContainer[idsindex];
...
private:
  ...
  const ZeroThickness setInterface(int iEl, const ES& entity_set, const E& element) const {
    ...
    ZeroThickness zt_element(length, topIndx_, butIndx_, intersection_index_, topMap, buttomMap, matchTopMap, matchButtomMap, topXip, buttomXip, topNormal, buttomNormal);

return zt_element;

    // zTContainer.insert(std::pair(iEl,zt_element));
    // zTBool[iEl] = true;
