...
public:
  ...
  */
  ...
  template<typename EG, typename IGS, typename LFSUC, typename LFSVC>
  void assembleZeroThickness(const EG & eg, const IGS & igs, const LFSUC & lfsu_cache, const LFSVC & lfsv_cache)
  {
    rl_view.setWeight(local_assembler.weight());
    Dune::PDELab::LocalAssemblerCallSwitch<LOP, LOP::doZeroThickness>::
    jacobian_apply_zero_thickness(lop, eg, igs, lfsu_cache.localFunctionSpace(), xl, lfsv_cache.localFunctionSpace(), rl_view);
  }
