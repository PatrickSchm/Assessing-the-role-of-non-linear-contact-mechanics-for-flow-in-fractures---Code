...
public:
  ...
  */
  ...
  {
    return prestage_engine->requireZeroThickness() || jacobian_engine->requireZeroThickness();
  }
  ...
  {
    if (prestage_engine->requireZeroThickness())
      prestage_engine->assembleZeroThickness(eg, igs, lfsu, lfsv, iMat);
    la.setWeight(-1.0);
    prestage_engine->setTimeInLastStage();
    if (jacobian_engine->requireZeroThickness())
      jacobian_engine->assembleZeroThickness(eg, igs, lfsu, lfsv, iMat);
  }
