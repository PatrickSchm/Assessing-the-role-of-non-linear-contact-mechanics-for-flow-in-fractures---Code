...
template<typename GFSU, typename GFSV, typename LOP,
         ...
         typename Grid, typename EIdx, typename BIdx = std::vector<int>,
...
public:
  ...
  typedef DefaultAssembler<GFSU, GFSV, CU, CV, Grid, EIdx> Assembler;
  ...
  GridOperator(const GFSU & gfsu_, const CU & cu_, const GFSV & gfsv_, const CV & cv_, LOP & lop_,
               const Grid grid, EIdx& element_id, BIdx& boundary_id, const MB& mb_ = MB())
    : global_assembler(gfsu_, gfsv_, cu_, cv_, grid, element_id, boundary_id)
  ...
  GridOperator(const GFSU & gfsu_, const GFSV & gfsv_, LOP & lop_, const std::vector<int>& element_id, const std::vector<int>& boundary_id, const MB& mb_ = MB())
