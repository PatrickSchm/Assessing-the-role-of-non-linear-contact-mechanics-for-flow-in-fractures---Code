...
namespace Dune {
  ...
  public:
    ...
    public:
      ...
      virtual Dune::FieldVector<double, dim+1> operator()(const Dune::FieldVector<double,dim>& local) const {
        ...
        psurface::StaticVector<double,dim> barCoords;
        ...
        psurface::StaticVector<double,dim+1> r;
    ...
    PSurfaceBoundary(psurface::PSurface<dim,double>* psurface)
      : psurface_(psurface)
    {}
     ...
     */
    psurface::PSurface<dim,double>* getPSurfaceObject()
    {
    ...
    {
      psurface::PSurface<dim,double>* newDomain;

#if HAVE_PSURFACE_2_0
      // Try to read the file as an hdf5 file
      if (filename.find(".h5")==filename.length()-3) {
        newDomain = psurface::Hdf5IO<double,dim>::read(filename);
        if (newDomain)
      ...
      newDomain
        = (psurface::PSurface<dim,double>*) psurface::AmiraMeshIO<double>::readAmiraMesh(am.get(), filename.c_str());
  ...
  private:
    ...
    std::unique_ptr<psurface::PSurface<dim,double> > psurface_;
