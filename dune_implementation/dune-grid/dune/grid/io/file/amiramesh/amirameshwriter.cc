                                              ...
                                              bool splitAll)
{
  ...
  for (; vertex!=endvertex; ++vertex) {
    ...
    // Copy coordinates
    for (int i=0; i<dim; i++)
      ((double*)geo_node_data->dataPtr())[dim*index+i] = vertex->geometry().corner(0)[i];
                                                    ...
                                                    bool gridSplitUp)
{
  ...
  // We are forced to use double for surface (i.e. 2d-in-3d) fields, because Amira
  // can only read those in double precision
  McPrimType primType = McPrimType::mc_double;
  if (std::is_same<ct,double>::value or (dim==2 and dimworld==3))
    primType = McPrimType::mc_double;
