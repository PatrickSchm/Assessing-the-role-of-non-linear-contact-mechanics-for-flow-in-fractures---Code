...
void Dune::AmiraMeshReader<GridType>::readFunction(DiscFuncType& f, const std::string& filename)
{
  ...
  if (!am->findData("Nodes", Hxdouble, blocksize, "Data") &&
      !am->findData("Nodes", HxDOUBLE, blocksize, "Data")) {

    // get the data field
    AmiraMesh::Data* am_ValueData =  am->findData("Nodes", Hxdouble, blocksize, "values");
    if (am_ValueData) {
      datafound = true;
      ...
      double* am_values_double = (double*) am_ValueData->dataPtr();

      for (i=0; i<am->nElements("Nodes"); i++) {
        for (j=0; j<blocksize; j++)
          f[i][j] = am_values_double[i*blocksize+j];
    ...
    // get the data field
    AmiraMesh::Data* am_ValueData =  am->findData("Nodes", Hxdouble, blocksize, "Data");
    if (am_ValueData) {
      datafound = true;
  ...
  if (!datafound and (am->findData("Triangles", Hxdouble, blocksize, "Data") or am->findData("Triangles", HxDOUBLE, blocksize, "Data")))
  {
    // get the data field
    AmiraMesh::Data* am_ValueData =  am->findData("Triangles", Hxdouble, blocksize, "Data");
    if (am_ValueData)
    {
      datafound = true;
                                                   ...
                                                   const std::shared_ptr<PSurfaceBoundary<dim-1> >& boundary)
{
  ...
  psurface::PSurface<2,double>* psurface = reinterpret_cast<psurface::PSurface<2,double>* >(boundary->getPSurfaceObject());
...
void Dune::AmiraMeshReader<GridType>::build2dGrid(GridFactory<GridType>& factory, AmiraMesh* am)
{
  ...
  AmiraMesh::Data* am_coordinateData =  am->findData("Nodes", Hxdouble, 2, "Coordinates");
  if (!am_coordinateData)
    DUNE_THROW(IOError, "2D AmiraMesh loader: field 'Nodes' not found!");

  double* am_node_coordinates = (double*) am_coordinateData->dataPtr();
                                                ...
                                                AmiraMesh* am)
{
  ...
  // get the different data fields
  AmiraMesh::Data* am_coordinateData =  am->findData("Nodes", Hxdouble, 3, "Coordinates");
  if (am_coordinateData)
  ...
  //   Insert interior nodes
  // //////////////////////////////////////
  assert(am_node_coordinates_double || am_node_coordinates_double);
  for(int i=0; i < noOfNodes; i++) {

    FieldVector<double,dimworld> nodePos;

    if (am_node_coordinates_double) {
