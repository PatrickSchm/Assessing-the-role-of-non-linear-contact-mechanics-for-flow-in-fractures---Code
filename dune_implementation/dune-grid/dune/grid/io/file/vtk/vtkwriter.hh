...
namespace Dune
{
  ...
  protected:
    ...
    public:
      ...
      typedef VTK::DataArrayWriter<double> Writer;
  ...
  private:
                             ...
                             const std::string& piecepath, const int commSize)
    {
           ...
           ++it)
      {
        ...
        writer.addArray<double>(it->name(), writecomps);
      }
      writer.endPointData();
      ...
      // PPoints
      writer.beginPoints();
      writer.addArray<double>("Coordinates", 3);
      writer.endPoints();
  ...
  protected:
    ...
    void writeData(VTK::VTUWriter& writer, const Data& data, const Iterator begin, const Iterator end, int nentries)
    {
           ...
           ++it)
      {
        ...
        std::shared_ptr<VTK::DataArrayWriter<double> > p
          (writer.makeArrayWriter<double>(f.name(), writecomps, nentries));
        if(!p->writeIsNoop())
    ...
    virtual void writeGridPoints(VTK::VTUWriter& writer)
    {
      ...
      std::shared_ptr<VTK::DataArrayWriter<double> > p
        (writer.makeArrayWriter<double>("Coordinates", 3, nvertices));
      if(!p->writeIsNoop()) {
