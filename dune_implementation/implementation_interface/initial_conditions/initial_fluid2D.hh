...
public:
    ...
    inline void evaluate(const I & intersection,
                         ...
                         BC::Type& y) const
    {
        Dune::FieldVector < typename I::ctype, I::dimension> xg =
            intersection.geometry().global(xlocal);

        if ((xg[0] >= 100.0 - 1.e-8 ) && (xg[0] <= 100.1 + 1.e-8 ) && (xg[1] <= 0.0 + 1.e-8 )) {
            y = BC::DoNothing;
            // y = BC::PressureDirichlet;
        } else if ((xg[1] <= 0.0 + 1.e-8 || xg[0] >= 201.0 - 1.e-8) && (xg[1] < 5.0)) {
            y = BC::PressureDirichlet;
...
public:
                         ...
                         typename Traits::RangeType& y) const
    {
        ...
        if ((xg[0] >= 100.0 - 1.e-8 ) && (xg[0] <= 100.1 + 1.e-8 ) && (xg[1] <= 0.0 + 1.e-8 )) {
            // && xg[0] >= 0.3 - 1.e-8) {
            y = 20000.0;
        } else if (xg[0] <= 0.0 + 1.e-8 || xg[0] >= 200.1 - 1.e-8) {
            y = 20000.0;
