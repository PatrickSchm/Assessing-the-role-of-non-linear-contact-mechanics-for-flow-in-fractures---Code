...
std::vector<double> delta_old(100000000, 0.0);

class deltaUpdate {
public:
    deltaUpdate (double length_,
...
void updateDelta(double dt, double delta_0, const GFS & gfs, const GRID & grid, U & unew, U & uold, EID & element_flags) {
    ...
    for (const auto & element : elements(entitySet))
    {
        ...
        for (const auto intersection : intersections(entitySet, element))
        {
            ...
            if (sqrt(pow(global0[0] - global1[0], 2) + pow(global0[1] - global1[1], 2)) < 1.1e-2) {
        ...
        const deltaUpdate zt_element(length, topMap, buttomMap, matchTopMap, matchButtomMap, topXip, buttomXip, topNormal, buttomNormal);
